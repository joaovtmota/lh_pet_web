...
{ 
    ...
    { 
        ...
        {
        var agendamentos = await _contexto.Agendamentos.Include(a => a.Pet).ThenInclude(p => p!.Cliente).OrderByDescending(a => a.DataHora).ToListAsync();
        ...
        {
            var agendamento = await _contexto.Agendamentos.Include(a => a.Pet).FirstOrDefaultAsync(a => a.Id == agendamentoId);
            if (agendamento == null) return NotFound();

            var atendimentoExistente = await _contexto.Atendimentos.FirstOrDefaultAsync(a => a.AgendamentoId == agendamentoId);
        ...
        {
            var query = _contexto.Atendimentos.Include(a => a.Agendamento).ThenInclude(ag => ag!.Pet).ThenInclude(p => p!.Cliente).AsQueryable();
        ...
        {
            ViewBag.PetsLista = await _contexto.Pets.Include(p => p.Cliente).Select(p => new {Id = p.Id, NomeExibicao = $"{p.Nome} (Tutor: {p.Cliente!.Nome})"}).ToListAsync();
