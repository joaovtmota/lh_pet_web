        ...
        [HttpPut("perfil")]
            public async Task<IActionResult> AtualizarPerfil([FromBody] ApiRegistroDTO dto)
            {
                ...
                if (usuario.Email != dto.Email)
                {
                    ...
                    if (emailEmUso) return BadRequest(new { Mensagem = "Este email já está em uso." });
                }
            ...
            public async Task<IActionResult> RemoverPet(int id)
            {
                ...
                var temAgendamentoPedente = await _contexto.Agendamentos.AnyAsync(a => a.PetId == pet.Id && a.Status == "Pendente");
                if (temAgendamentoPedente) return BadRequest(new { mensagem = "Não é possível remover o pet, pois ele possui agendamentos pedentes." });
            ...
            public async Task<IActionResult> ListarAgendamentos()
            {
                var cliente = await ObterClienteLogadoAsync();
                if (cliente == null) return Unauthorized();
                var agendamentos = await _contexto.Agendamentos.Include(a => a.Pet).Where(a => a.Pet!.ClienteId == cliente.Id).OrderByDescending(a => a.DataHora).Select(a => new { a.Id, PetNome = a.Pet!.Nome, a.DataHora, a.Tipo, a.Status }).ToListAsync();
            ...
            public async Task<IActionResult> CriarAgendamento([FromBody] ApiAgendamentoDTO dto)
            {
                ...
                {
                    PetId = dto.PetId, DataHora = dto.DataHora, Tipo = dto.Tipo, Status = "Pendente"
                };
